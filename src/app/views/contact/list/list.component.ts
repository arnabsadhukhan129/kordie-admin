...
export class ListComponent implements OnInit, OnDestroy {
  ...
  contactDeleteSubs!: Subscription;
  totalRecords!: number;
  ...
  ngOnDestroy(): void {
    ...
    if(this.contactDeleteSubs){
      this.contactDeleteSubs.unsubscribe();
    }
  }
  ...
  getList(page = this.current, limit = this.recordsPerPage) {
    ...
    this.contactSubs = this._contact.getList(params, this.searchType, this.searchVal)
      .subscribe((response)=>{
        if(response.error == false)
        {
          this.listData = response?.data?.contact;
      ...
      if (confirm("Are you sure to delete?") == true) 
        {
            this.contactDeleteSubs = this._contact.delete(id)
              .subscribe((response)=>{
