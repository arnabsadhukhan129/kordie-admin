...
export class ListComponent implements OnInit, OnDestroy {
  cutaorsPage!: Subscription;
  cutaorsSubscription!: Subscription;
  deleteCutaorSubscription!: Subscription;
  current: number = 1;
  ...
  ngOnInit(): void {
    this.cutaorsPage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  ngOnDestroy(): void {
    if(this.cutaorsPage){
      this.cutaorsPage.unsubscribe();
    }
    if(this.cutaorsSubscription){
      this.cutaorsSubscription.unsubscribe();
    }
    if(this.deleteCutaorSubscription){
      this.deleteCutaorSubscription.unsubscribe();
    }
  }
  ...
  getList(page = this.current, limit = this.recordsPerPage) {
    ...
    this.cutaorsSubscription = this._cutaors.getList(params, this.searchType, this.searchVal)
                .subscribe((response)=>{
  ...
  delete(id: string){
    if (confirm("Are you sure to delete?") == true) 
      {
              this.deleteCutaorSubscription =   this._cutaors.delete(id)
              .subscribe((response)=>{
