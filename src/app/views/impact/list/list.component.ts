...
export class ListComponent implements OnInit, OnDestroy {
  imapactSubs!: Subscription;
  impactPage!: Subscription;
  totalRecords!: number;
  ...
  ngOnInit(): void {
    this.impactPage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  ngOnDestroy(): void {
    if(this.impactPage){
      this.impactPage.unsubscribe();
    }
    if(this.imapactSubs){
      this.imapactSubs.unsubscribe();
    }
  }
  ...
  showLoader(){
    ...
    getList(page = this.current, limit = this.recordsPerPage) {
      ...
      this.imapactSubs = this.__impact.getList(params, this.searchType, this.searchVal)
          .subscribe((response)=>{
