...
export class ListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.blogTypePage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  ngOnDestroy(): void {
    if(this.blogTypePage){
      this.blogTypePage.unsubscribe();
    }
  }
  ...
  //List..............
    getList(page: number = this.current, limit: number = this.recordsPerPage): void {
      ...
      this._blogType.blogTypeList(params, this.searchType, this.searchVal).subscribe(
        (response) => {
          // Stop the loader
          this.__spinner.hide();
      ...
      toggleStatus(item: any): void {
        ...
        this._updateStatus.blogTypeStatus(item._id).subscribe({
          next: (response) => {
            if (response.error) {
      ...
      delete(id: string){
        if (confirm("Are you sure to delete?") == true) 
          {
                    this._updateStatus.blogTypeDelete(id)
                  .subscribe((response)=>{
