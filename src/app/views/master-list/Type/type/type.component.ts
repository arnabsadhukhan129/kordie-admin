...
export class TypeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.typePage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  getList(page = this.current, limit = this.recordsPerPage) {
    ...
    this._type.typeList(params, this.searchType, this.searchVal)
      .subscribe((response)=>{
        if(response.error == false)
        {
          this.listData = response?.data?.items;
          // console.log("typeList.....",this.listData);
  ...
  toggleStatus(item: any): void {
    ...
    this._updateStatus.typeStatus(item._id).subscribe({
      next: (response) => {
        if (response.error) {
  ...
  delete(id: string){
    if (confirm("Are you sure to delete?") == true) 
      {
          this._updateStatus.typeDelete(id)
            .subscribe((response)=>{
