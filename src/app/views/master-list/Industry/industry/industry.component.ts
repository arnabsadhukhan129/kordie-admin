...
export class IndustryComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.industryPage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  getList(page = this.current, limit = this.recordsPerPage) {
    ...
    this._industry.industryList(params, this.searchType, this.searchVal)
      .subscribe((response)=>{
  ...
  toggleStatus(item: any): void {
    ...
    this._updateStatus.industryStatus(item._id).subscribe({
      next: (response) => {
        if (response.error) {
  ...
  delete(id: string){
    if (confirm("Are you sure to delete?") == true) 
      {
                this._updateStatus.industryDelete(id)
              .subscribe((response)=>{
