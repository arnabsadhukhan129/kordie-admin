...
export class InterestComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.interestPage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  ngOnDestroy(): void {
    if(this.interestPage){
      this.interestPage.unsubscribe();
    }
  }
  ...
  getList(page = this.current, limit = this.recordsPerPage) {
    ...
    this._interest.interestList(params, this.searchType, this.searchVal)
      .subscribe((response)=>{
  ...
  toggleStatus(item: any): void {
    ...
    this._updateStatus.interestStatus(item._id).subscribe({
      next: (response) => {
        if (response.error) {
  ...
  delete(id: string){
    if (confirm("Are you sure to delete?") == true) 
      {
                this._updateStatus.interestDelete(id)
              .subscribe((response)=>{
