...
export class ListComponent implements OnInit, OnDestroy {
  studenSpeakPage!: Subscription;
  speakSubscription!: Subscription;
  deleteSpeakSubscription!: Subscription;
  current: number = 1;
  ...
  ngOnInit(): void {
    this.studenSpeakPage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  ngOnDestroy(): void {
    if(this.studenSpeakPage){
      this.studenSpeakPage.unsubscribe();
    }
    if(this.speakSubscription){
      this.speakSubscription.unsubscribe();
    }
    if(this.deleteSpeakSubscription){
      this.deleteSpeakSubscription.unsubscribe();
    }
  }
  ...
  getList(page = this.current, limit = this.recordsPerPage) {
    ...
    this.speakSubscription = this._speak.getList(params, this.searchType, this.searchVal)
                .subscribe((response)=>{
  ...
  delete(id: string){
    if (confirm("Are you sure to delete?") == true) 
      {
              this.deleteSpeakSubscription = this._speak.delete(id)
              .subscribe((response)=>{
