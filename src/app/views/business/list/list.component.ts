...
export class ListComponent implements OnInit, OnDestroy {
  businessPage!: Subscription;
  businessSubs!: Subscription;
  businessDeleteSubs!: Subscription;
  current: number = 1;
  ...
  ngOnInit(): void {
    this.businessPage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  ngOnDestroy(): void {
    if(this.businessPage){
      this.businessPage.unsubscribe();
    }
    if(this.businessSubs){
      this.businessSubs.unsubscribe();
    }
    if(this.businessDeleteSubs){
      this.businessDeleteSubs.unsubscribe();
    }
  }
  ...
  getList(page = this.current, limit = this.recordsPerPage) {
    ...
    this.businessSubs = this.__business.getList(params, this.searchType, this.searchVal)
        .subscribe((response)=>{
   ...
   sanitizeHtml(htmlContent: string): SafeHtml {
    ...
    delete(id: string){
      if (confirm("Are you sure to delete?") == true) 
        {
              this.businessDeleteSubs = this.__business.delete(id)
                .subscribe((response)=>{
