...
export class ListComponent implements OnInit, OnDestroy {
  ...
  blogDeleteSubscription!: Subscription;
  blogStatusSubscription!: Subscription;
  blogPage!: Subscription;
  totalRecords!: number;
  ...
  ngOnInit(): void {
    ...
    this.isLoading = true;
    this.__spinner.show();
    this.blogPage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  ngOnDestroy(): void {
    if(this.blogPage){
      this.blogPage.unsubscribe();
    }
    if(this.blogListSubscription){
      this.blogListSubscription.unsubscribe();
    }
    if(this.blogDeleteSubscription){
      this.blogDeleteSubscription.unsubscribe();
    }
    if(this.blogStatusSubscription){
      this.blogStatusSubscription.unsubscribe();
    }
  }
  ...
  getList(page = this.current, limit = this.recordsPerPage) {
    ...
    this.blogListSubscription = this._blog.getList(params, this.searchType, this.searchVal)
      .subscribe((response)=>{
        if(response.error == false)
        {
          this.listData = response?.data?.blog;
  ...
  delete(id: string){
    if (confirm("Are you sure to delete?") == true) 
      {
            this.blogDeleteSubscription =  this._blog.delete(id)
              .subscribe((response)=>{
  ...
  toggleStatus(item: any): void {
    const previousStatus = item.featured;
    item.featured = !item.featured;
  
    this.blogStatusSubscription = this._blog.featureStatus(item._id).subscribe({
      next: (response) => {
        if (response.error) {
          this.handleError(response.message);
          item.featured = previousStatus;
        } else {
