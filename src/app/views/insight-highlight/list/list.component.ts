...
import { InsightArticleService } from '../../../../app/services/Insight-highlight/insight-article.service';
...
export class ListComponent implements OnInit, OnDestroy {
  insightsPage!: Subscription;
  insightSubscription!: Subscription;
  deleteInsightsubscription!: Subscription
  current: number = 1;
  ...
  ngOnInit(): void {
    this.insightsPage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  ngOnDestroy(): void {
    if(this.insightsPage){
      this.insightsPage.unsubscribe();
    }
    if(this.insightSubscription){
      this.insightSubscription.unsubscribe();
    }
    if(this.deleteInsightsubscription){
      this.deleteInsightsubscription.unsubscribe();
    }
  }
  ...
  getList(page = this.current, limit = this.recordsPerPage) {
    ...
    this.insightSubscription = this._insights.getList(params, this.searchType, this.searchVal)
                .subscribe((response)=>{
  ...
  delete(id: string){
    if (confirm("Are you sure to delete?") == true) 
      {
              this.deleteInsightsubscription = this._insights.delete(id)
              .subscribe((response)=>{
