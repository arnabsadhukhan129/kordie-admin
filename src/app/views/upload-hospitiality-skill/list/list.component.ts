...
import { UploadService } from 'src/app/services/upskill/upload.service';
...
export class ListComponent implements OnInit {
  ...
  constructor(
    private __upload: UploadService,
  ...
  ngOnInit(): void {
    this.uploadPage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  ngOnDestroy(): void {
   ...
   getList(page = this.current, limit = this.recordsPerPage) {
    ...
    this.__upload.getList(params, this.searchType, this.searchVal)
                .subscribe((response)=>{
                  if(response.error == false)
                  {
                    this.listData = response.data?.focus;
