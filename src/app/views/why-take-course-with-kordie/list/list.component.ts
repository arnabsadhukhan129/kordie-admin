...
export class ListComponent implements OnInit {
  deleteTakeCourse!: Subscription;
  takeCourseSubs!: Subscription;
  takeCoursePage!: Subscription;
  totalRecords!: number;
  ...
  ngOnInit(): void {
    this.takeCoursePage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  ngOnDestroy(): void {
      if(this.takeCoursePage){
        this.takeCoursePage.unsubscribe();
      }
      if(this.takeCourseSubs){
        this.takeCourseSubs.unsubscribe()
      }
      if(this.deleteTakeCourse){
        this.deleteTakeCourse.unsubscribe();
      }
  }
  ...
  showLoader(){
    ...
    getList(page = this.current, limit = this.recordsPerPage) {
      ...
      this.takeCourseSubs = this._takeCourse.getList(params, this.searchType, this.searchVal)
        .subscribe((response)=>{
    ...
    delete(id: string){
      if (confirm("Are you sure to delete?") == true) 
        {
                this.deleteTakeCourse = this._takeCourse.delete(id)
                .subscribe((response)=>{
