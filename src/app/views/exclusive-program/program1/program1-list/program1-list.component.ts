...
@Component({
  selector: 'app-program1-list',
  templateUrl: './program1-list.component.html',
  styleUrls: ['./program1-list.component.scss']
})
export class Program1ListComponent implements OnInit, OnDestroy {
  exclusiveProgram1Subs!: Subscription;
  program1Page!: Subscription;
  exclusiveProgram1DeleteSubs!: Subscription;
  current: number = 1;
  ...
  ngOnInit(): void {
    this.program1Page = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  ngOnDestroy(): void {
    if(this.program1Page){
      this.program1Page.unsubscribe();
    }
    if(this.exclusiveProgram1Subs){
      this.exclusiveProgram1Subs.unsubscribe()
    }
    if(this.exclusiveProgram1DeleteSubs){
      this.exclusiveProgram1DeleteSubs.unsubscribe();
    }
  }
  ...
  //List Data................
    getList(page = this.current, limit = this.recordsPerPage) {
      ...
      this.exclusiveProgram1Subs = this.__program1.getList(params, this.searchType, this.searchVal)
          .subscribe((response)=>{
     ...
     sanitizeHtml(htmlContent: string): SafeHtml {
      ...
      delete(id: string){
        if (confirm("Are you sure to delete?") == true) 
          {
            this.exclusiveProgram1DeleteSubs = this.__program1.delete(id)
            .subscribe((response)=>{
