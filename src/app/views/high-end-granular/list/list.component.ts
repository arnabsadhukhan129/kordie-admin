...
export class ListComponent implements OnInit,OnDestroy {
  ...
  ngOnInit(): void {
    this.deliveryPage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  getList(page = this.current, limit = this.recordsPerPage) {
    ...
    this._granular.getList(params, this.searchType, this.searchVal)
                .subscribe((response)=>{
                  if(response.error == false)
                  {
                    this.listData = response.data.delivery;
