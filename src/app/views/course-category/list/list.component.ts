...
export class ListComponent implements OnInit {
  ...
  courseCategoryPage!: Subscription;
  courseToggleSubscription!: Subscription;
  ...
  ngOnInit(): void {
    this.courseCategoryPage = this._pageService.getCurrentPage().subscribe((page:any)=>{
      if(page){
        this.current = page
      }
      else{
  ...
  ngOnDestroy(): void {
    ...
    if(this.courseCategoryPage){
      this.courseCategoryPage.unsubscribe();
    }
    if(this.courseCategorySubscription){
      this.courseCategorySubscription.unsubscribe();
    }
    if(this.courseToggleSubscription){
      this.courseToggleSubscription.unsubscribe();
    }
  ...
  getList(page = this.current, limit = this.recordsPerPage) {
    ...
    this.courseCategorySubscription = this._category.getList(params, this.searchType, this.searchVal)
      .subscribe((response)=>{
  ...
  toggleStatus(item: any): void {
    ...
    this.courseToggleSubscription = this._category.updateStatus(item._id).subscribe({
      next: (response) => {
        if (response.error) {
  ...
  delete(id: string){
    if (confirm("Are you sure to delete?") == true) 
      {
            this.deleteCategorySubscription = this._category.delete(id)
              .subscribe((response)=>{
